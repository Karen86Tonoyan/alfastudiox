...
export interface CloudBridgeSection {
  provider_id: string;
  sync_enabled: boolean;
}

export interface HybridBurstSection {
  enabled: boolean;
  strategy: "local_first" | "cloud_preferred";
  cloud_provider_id: string;
  burst_on_local_saturation: boolean;
  burst_on_master_drain: boolean;
}
...
export interface ControllerConfig {
  ...
  cloud_bridge: CloudBridgeSection;
  hybrid_burst: HybridBurstSection;
}
...
export const DEFAULT_CONFIG: ControllerConfig = {
  ...
  hybrid_burst: {
    ...
    cloud_provider_id: "runpod",
    burst_on_local_saturation: true,
    burst_on_master_drain: true,
  },
};
...
function sanitizeNode(raw: unknown, idx: number): NodeConfig {
  ...
  const node: NodeConfig = {
    ...
    api_url: str(r.api_url, "http://127.0.0.1:8000", 512),
...
export function sanitizeConfig(raw: unknown): ControllerConfig {
  ...
  const cb = isObj(r.cloud_bridge) ? r.cloud_bridge : {};
  const hb = isObj(r.hybrid_burst) ? r.hybrid_burst : {};
  ...
  return {
    ...
    cloud_bridge: {
      provider_id: str(cb.provider_id, d.cloud_bridge.provider_id, 64) || d.cloud_bridge.provider_id,
      sync_enabled: bool(cb.sync_enabled, d.cloud_bridge.sync_enabled),
    },
    hybrid_burst: {
      enabled: bool(hb.enabled, d.hybrid_burst.enabled),
      strategy: oneOf(hb.strategy, ["local_first", "cloud_preferred"] as const, d.hybrid_burst.strategy),
      cloud_provider_id: str(hb.cloud_provider_id, d.hybrid_burst.cloud_provider_id, 64) || d.hybrid_burst.cloud_provider_id,
      burst_on_local_saturation: bool(hb.burst_on_local_saturation, d.hybrid_burst.burst_on_local_saturation),
      burst_on_master_drain: bool(hb.burst_on_master_drain, d.hybrid_burst.burst_on_master_drain),
    },
  };
}
