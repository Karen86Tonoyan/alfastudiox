...
import type { NodeMode, RuntimeCapability } from "@/lib/runtime/types/nodeHealth";
...
const DEFAULT_RUNTIME_CAPABILITY: RuntimeCapability = {
  providerVersion: "comfy-cluster@1",
  supportedFeatures: ["submit", "snapshotSync", "cloudBridge"],
  runtimeLevel: 1,
};
...
class ClusterManager {
  ...
  getNodes(): ClusterNodeRuntime[] {
    return this.nodes.map((n) => ({
      ...n,
      status: "offline",
      mode: n.enabled ? "ACTIVE" : "OFFLINE",
      vramUsed: 0,
      vramTotal: 0,
      gpuTemp: 0,
      queueSize: 0,
      gpuName: "",
      runtimeCapability: DEFAULT_RUNTIME_CAPABILITY,
  ...
  setNodeMode(id: string, mode: NodeMode) {
  ...
  pauseNode(id: string) {
    this.setNodeMode(id, "DEGRADED");
  }

  drainJobs(id: string) {
    this.setNodeMode(id, "DRAINING");
  }

  resumeNode(id: string) {
    const node = this.nodes.find((entry) => entry.id === id);
    this.setNodeMode(id, node?.enabled === false ? "OFFLINE" : "ACTIVE");
  }

  upgradeRuntimeLevel(id: string, patch: Partial<RuntimeCapability>) {
    const current = this.runtime.get(id)?.runtimeCapability || DEFAULT_RUNTIME_CAPABILITY;
    this.runtime.set(id, {
      ...this.runtime.get(id),
      runtimeCapability: {
        ...current,
        ...patch,
        supportedFeatures: patch.supportedFeatures ?? current.supportedFeatures,
      },
      lastSeen: Date.now(),
    });
    this.notify();
  ...
  async dispatch(
    ...
    if (opts.forceNodeId) {
      ...
      const node = this.getNodes().find((n) => n.id === opts.forceNodeId);
      if (!c || !node) return { nodeId: "", nodeName: "", promptId: null, delegated: false, reason: "Node not found" };
      if (node.mode !== "ACTIVE") {
        return { nodeId: node.id, nodeName: node.name, promptId: null, delegated: false, reason: `Node mode ${node.mode} blocks new jobs` };
      }
    ...
    if (master.mode !== "ACTIVE") {
      ...
      if (w) {
        ...
        return {
          ...
          promptId,
          delegated: true,
          reason: `Master mode ${master.mode} -> delegated to ${w.name}`,
        };
      }
    }
