...
function rasterizeLayer(layer: EditorLayer): HTMLCanvasElement {
  const width = Math.max(1, Math.round(layer.canvas.width * layer.scaleX));
  const height = Math.max(1, Math.round(layer.canvas.height * layer.scaleY));
  ...
  if (layer.maskCanvas) {
    ...
    maskCtx.drawImage(layer.maskCanvas, 0, 0, width, height);
...
export function exportPSD(
  ...
  const children: Layer[] = layers.map((layer) => {
    const rasterized = rasterizeLayer(layer);
    const l: Layer = {
      name: layer.name,
      ...
      left: layer.x,
      top: layer.y,
      right: layer.x + rasterized.width,
      bottom: layer.y + rasterized.height,
      canvas: rasterized as unknown as HTMLCanvasElement,
    };
  ...
  for (const layer of layers) {
    ...
    const rasterized = rasterizeLayer(layer);
    ctx.drawImage(rasterized, layer.x, layer.y);
    ctx.restore();
  }
