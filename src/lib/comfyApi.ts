...
const DEFAULT_COMFY_CANDIDATES = [
  "127.0.0.1:8000",
  "localhost:8000",
  "localhost:8188",
  "127.0.0.1:8188",
...
async function probeComfyHttp(baseUrl: string): Promise<boolean> {
  const clean = normalizeBaseUrl(baseUrl);
  const protocol = clean.includes("ngrok") || clean.includes(".app") ? "https" : "http";
  try {
    const res = await fetch(`${protocol}://${clean}/system_stats`, {
...
export class ComfyApiClient extends EventEmitter {
  ...
  constructor(baseUrl?: string) {
    super();
    this._baseUrl = normalizeBaseUrl(baseUrl || localStorage.getItem("comfy_server_url") || DEFAULT_COMFY_CANDIDATES[0]);
  }

  get baseUrl() { return this._baseUrl; }
  set baseUrl(url: string) {
    this._baseUrl = normalizeBaseUrl(url);
    localStorage.setItem("comfy_server_url", this._baseUrl);
  }
  ...
  private async resolveBaseUrl(preferredUrl?: string): Promise<string> {
    const saved = localStorage.getItem("comfy_server_url");
    const candidates = [
      preferredUrl,
      this._baseUrl,
      saved || undefined,
      ...DEFAULT_COMFY_CANDIDATES,
    ]
    ...
    for (const candidate of candidates) {
      if (await probeComfyHttp(candidate)) {
        this.baseUrl = candidate;
        return candidate;
      }
    }

    return normalizeBaseUrl(preferredUrl || this._baseUrl);
  }

  async connect(preferredUrl?: string) {
    if (this._status === "connected" || this._status === "connecting") return;
    this._status = "connecting";
    this.emit("status", this._status);

    try {
      const resolvedBaseUrl = await this.resolveBaseUrl(preferredUrl);
